...
function assert(boolean, message, errorType = null) {
	if (boolean) return;
	if (errorType !== null && (errorType === Error || Error.isPrototypeOf(errorType))) throw new errorType(message);
...
assert.object = (input, argName) => assert.argType(input !== null && (typeof (input) === "object" && !Array.isArray(input)), "an Object", argName);
assert.iterable = (input, argName) => assert.argType(input !== null && ((typeof input) === "object") && (Symbol.iterator in input), "iterable", argName);
/**
...
LinkedList.prototype.assertElement = function (input, argName) {
	assert.argType(input instanceof this.ListElement, "ListElement", argName);
};
